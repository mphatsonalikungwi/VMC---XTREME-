...
## Visual redesign (round 2 — replacing the generic AI-look palette)

- **Palette replaced entirely.** The old near-black background + bright red/orange gradient accent is one of the most recognizable AI-generated design defaults. Replaced with a materials-grounded industrial palette: `#1a1a17` charcoal (gym-floor, not pure black), `#242420` panel, `#ede8dc` chalk-white text, `#d4a017` iron-gold (like paint on a weight plate), `#8c2f1f` oxidized-iron red (used sparingly, not everywhere). Every hardcoded rgba literal using the old colors was found and replaced too, not just the CSS variables — a simple variable swap would have missed dozens of hover/border states.
- **Typography** — added Oswald (bold condensed, industrial/stencil feel) for all headings, kept Inter for body copy since it was already a sound, legible choice for mobile.
- **Removed all emoji from the public-facing page** — service icons, contact section, payment badges, hamburger menu, and the floating WhatsApp button now use custom hand-drawn SVG line icons instead. Verified via a full character scan of the file: zero emoji remain outside the owner-only dashboard template (which visitors never see).
- **Removed the fake "01–06" sequence numbers** on service cards — your six programs aren't a step-by-step process, so numbering them implied an order that isn't real.
- **Redesigned the pricing "Featured" badge** from a generic rounded SaaS pill into a rotated, stamped-tag style badge reading "Most Chosen" — ties back to the physical, tactile feel of an actual gym rather than a software product.
- **Removed 5 stray, unused duplicate image files** sitting loose in the repo root (leftover from an earlier upload mix-up) — `index.html` only ever referenced the copies inside `/assets`, so the root-level ones were dead weight.
- Verified after every batch of changes by actually running the page's JavaScript in a real engine and clicking Join Now / the nav menu — not just eyeballing the HTML. Confirmed working both before and after the full redesign pass.
...
- Confirm the correct live Render service URL (`vmc-xtreme-1` vs `vmc-xtreme-api`) — see security section above.
- Confirm whether Join Now / nav were actually broken, or just a browser-cache issue on the previous stale deploy.
