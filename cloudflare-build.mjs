...
if(!app.includes('amountForDuration')){
 ...
 app=app.replace("const amount=state.selectedPlan?.price||priceFor(d.membership_tier,d.session_type);", "const count=Math.floor(Number(d.duration_count||1)),unit=String(d.duration_unit||'month'),amount=state.selectedPlan?.price||amountForDuration(d.membership_tier,d.session_type,count,unit);");
