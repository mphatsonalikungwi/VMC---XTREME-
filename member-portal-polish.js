...
const boot=()=>{
 ...
 const profile=$('#vmcProfileView'),gallery=$('#vmcGalleryView'),settings=$('#vmcSettingsView');
 let home=$('#vmcPortalHome');
 const logoUrl=new URL('assets/logo.png',window.location.origin).href;
 const style=document.createElement('style');
 style.id='vmc-portal-safe-layout';
 style.textContent=`
 ...
 .bar{height:84px!important;max-width:1240px!important;display:flex!important;align-items:center!important}
 ...
 .brand img{display:block!important;width:164px!important;height:auto!important;max-height:62px!important;object-fit:contain!important}
 ...
 .vmc-portal-home,.view{display:none!important}
 .vmc-portal-home.vmc-view-active,.view.vmc-view-active{display:block!important}
 ...
 .vmc-menu-toggle{display:inline-flex!important;align-items:center!important;justify-content:center!important;width:46px!important;height:46px!important;margin-left:auto!important;border:1px solid var(--vmc-line)!important;border-radius:12px!important;background:#fff!important;color:var(--vmc-ink)!important;font-size:24px!important;cursor:pointer!important}
 ...
 .vmc-home-main{display:grid;grid-template-columns:minmax(0,1.3fr) minmax(240px,.7fr);gap:28px;align-items:center;padding:42px;background:linear-gradient(120deg,#edf8f6 0%,#fff 64%)}
 ...
 .avatar{background:var(--vmc-teal-soft)!important;color:var(--vmc-teal-dark)!important}.btn{background:var(--vmc-teal)!important;color:#fff!important;border:0!important}.field input{background:#fff!important;color:var(--vmc-ink)!important;border-color:#cbd8dc!important}
 @media(max-width:800px){.content{padding:16px!important}.vmc-home-main{grid-template-columns:1fr;padding:28px 24px;gap:22px}.vmc-home-member{order:-1;align-items:flex-start;text-align:left}.vmc-home-avatar{width:104px;height:104px}.vmc-home-summary{grid-template-columns:1fr 1fr}.vmc-home-stat:last-child{grid-column:1/-1}.grid{grid-template-columns:1fr 1fr!important}.subgrid,.settings-grid{grid-template-columns:1fr!important}}
 @media(max-width:520px){.top,.bar{height:76px!important}.brand img{width:137px!important}.side{top:76px;height:calc(100vh - 76px)}.vmc-menu-backdrop{inset:76px 0 0}.vmc-home h1{font-size:37px}.vmc-home p{font-size:14px}.vmc-home-summary{grid-template-columns:1fr}.vmc-home-stat:last-child{grid-column:auto}.grid{grid-template-columns:1fr!important}}
 `;
 document.getElementById(style.id)?.remove();document.head.appendChild(style);
 const brand=document.querySelector('.brand');
 if(brand)brand.innerHTML='<img src="'+logoUrl+'" alt="VMC Xtreme Fitness logo">';
 let toggle=document.querySelector('.vmc-menu-toggle');
 ...
 let backdrop=document.querySelector('.vmc-menu-backdrop');
 ...
 toggle.onclick=e=>{e.preventDefault();e.stopPropagation();const open=!side.classList.contains('vmc-open');side.classList.toggle('vmc-open',open);backdrop.classList.toggle('open',open);toggle.textContent=open?'×':'☰';toggle.setAttribute('aria-label',open?'Close navigation menu':'Open navigation menu')};
 backdrop.onclick=close;
 const show=name=>{allViews().forEach(v=>{v.classList.remove('vmc-view-active','active');v.style.display='none'});const target=name==='home'?home:{profile,gallery,settings}[name];if(target){target.classList.add('vmc-view-active');target.style.display='block'}side.querySelectorAll('[data-vmc-view]').forEach(b=>b.classList.toggle('active',b.dataset.vmcView===name));close();if(name==='gallery'&&typeof window.vmcLoadGallery==='function')window.vmcLoadGallery()};
 side.querySelectorAll('[data-vmc-view]').forEach(b=>{b.onclick=e=>{e.preventDefault();e.stopPropagation();show(b.dataset.vmcView)}});
 content.querySelectorAll('[data-home-view]').forEach(b=>b.onclick=()=>show(b.dataset.homeView));
 const copyProfileToHome=()=>{const name=$('#vmcName')?.textContent?.trim()||$('#vmcWelcomeName')?.textContent?.trim()||'Member';const username=$('#vmcUsername')?.textContent?.trim()||'@username';const plan=$('#vmcPlan')?.textContent?.trim()||$('#vmcPlanCard')?.textContent?.trim()||'—';const status=$('#vmcStatus')?.textContent?.trim()||'Checking…';const expiry=$('#vmcExpiry')?.textContent?.trim()||'—';if($('#vmcHomeName'))$('#vmcHomeName').textContent=name.split(' ')[0];if($('#vmcHomeMemberName'))$('#vmcHomeMemberName').textContent=name;if($('#vmcHomeUsername'))$('#vmcHomeUsername').textContent=username;if($('#vmcHomePlan'))$('#vmcHomePlan').textContent=plan;if($('#vmcHomeStatus'))$('#vmcHomeStatus').textContent=status;if($('#vmcHomeRemaining'))$('#vmcHomeRemaining').textContent=expiry==='—'?'—':'Until '+expiry;const source=$('#vmcAvatar img'),av=$('#vmcHomeAvatar');if(av&&source&&source.src){if(av.dataset.source!==source.src){av.innerHTML='<img src="'+source.src+'" alt="Profile photo">';av.dataset.source=source.src}}else if(av&&!av.querySelector('img'))av.textContent=(name.match(/[A-Za-z]/)||['V'])[0].toUpperCase()};
 show('home');copyProfileToHome();
 setTimeout(copyProfileToHome,800);setTimeout(copyProfileToHome,2000);setInterval(copyProfileToHome,5000);
};
