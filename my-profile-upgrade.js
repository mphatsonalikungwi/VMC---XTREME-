...
const installMyProfile=()=>{
  ...
  const anniversary=document.createElement('div');anniversary.className='vmc-profile-anniversary';anniversary.innerHTML='<small id="vmcProfileAnniversaryLabel">YOUR VMC JOURNEY</small><strong id="vmcProfileAnniversaryTitle">Your VMC journey is just beginning.</strong><p id="vmcProfileAnniversaryText">Your member-since date will be used to celebrate your VMC milestones automatically.</p>';
  ...
  const getAnniversary=(value,name)=>{
    if(!value)return{label:'YOUR VMC JOURNEY',title:'Your VMC journey is just beginning.',text:'Your member-since date will be used to celebrate your VMC milestones automatically.',milestone:'Starting your journey'};
    const start=new Date(value);if(Number.isNaN(start.getTime()))return{label:'YOUR VMC JOURNEY',title:'Your VMC journey is underway.',text:`Welcome to VMC, ${name}.`,milestone:'VMC member'};
    ...
    if(years<1){const startDay=new Date(start.getFullYear(),start.getMonth(),start.getDate());const days=Math.max(0,Math.floor((new Date(now.getFullYear(),now.getMonth(),now.getDate())-startDay)/86400000));return{label:'YOUR VMC JOURNEY',title:'Your VMC journey is just beginning.',text:`Your VMC journey began ${days} day${days===1?'':'s'} ago.`,milestone:`${days} day${days===1?'':'s'} with VMC`}}
  ...
  const load=async()=>{
    try{
      ...
      const a=getAnniversary(p.created_at,first);set('vmcProfileAnniversaryLabel',a.label,'YOUR VMC JOURNEY');set('vmcProfileAnniversaryTitle',a.title,'Your VMC journey is underway.');set('vmcProfileAnniversaryText',a.text,'Keep showing up and keep improving.');set('vmcProfileMilestone',a.milestone,'VMC member');
